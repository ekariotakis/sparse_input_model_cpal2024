...
import cls_data_loader as cls_dataset
import transform_data_loader as transform_dataset
...
##############  # 0 for cls_dataset, 1 for transform_dataset
select_dataset = 0
##############
...
#     def __init__(self, partition_num=1, sample_size=4096, model_size=4096, label_num=35):
    def __init__(self, partition_num=1, sample_size=768, model_size=4096, label_num=10):
        ...
        self.fc1 = nn.Linear(sample_size, model_size)
        self.fc2 = nn.Linear(model_size, model_size)
        self.fc3 = nn.Linear(model_size, label_num)
...
def train(args, partitioned_model, raw_model, optimizer, train_loader, epoch, train_time_log):
    ...
    for i, (x,y) in enumerate(train_loader):
        # Disable gradient tracking for input tensors
...
def test(args, raw_model, test_loader, epoch, test_loss_log, test_acc_log):
    ...
    with torch.no_grad():
        for _, (x,y) in enumerate(test_loader):
            ...
            criterion = nn.CrossEntropyLoss()        
            test_loss += criterion(output, y)  # sum up batch loss
...
def worker_process(args):
    ...
    if select_dataset == 0:
        train_set = cls_dataset.train_dataset(args.mask_ratio, args.dataset)
    else:
        train_set = transform_dataset.train_dataset()
    train_loader = DataLoader(train_set, batch_size=args.batch_size, shuffle=True, num_workers=4, pin_memory=True,
                              drop_last=True)
    if args.dataset == 'CIFAR10':
        partitioned_model = MLP3Layer(partition_num=1, model_size=args.model_size // args.world_size).to(device)
    elif args.dataset == 'RESISC45':
        partitioned_model = MLP3Layer(partition_num=1, model_size=args.model_size // args.world_size, label_num=45).to(device)
    elif args.dataset == 'AID':
        partitioned_model = MLP3Layer(partition_num=1, model_size=args.model_size // args.world_size, label_num=30).to(device)
    optimizer = torch.optim.Adam(partitioned_model.parameters())
...
def parameter_server_process(args):
    ...
    if select_dataset == 0:
        train_set = cls_dataset.train_dataset(args.mask_ratio, args.dataset)
        test_set = cls_dataset.test_dataset(args.mask_ratio, args.dataset)
    else:
        train_set = transform_dataset.train_dataset()
        test_set = transform_dataset.test_dataset()
    train_loader = DataLoader(train_set, batch_size=args.batch_size, shuffle=True, num_workers=4, pin_memory=True,
                              drop_last=True)
    test_loader = DataLoader(test_set, batch_size=args.batch_size, shuffle=True, num_workers=4, pin_memory=True,
                             drop_last=False)
    model_name = 'MLP_3_layer_' + str(args.epochs) + '_' + str(args.model_size) \
                 + '_cascaded_' + str(args.world_size) + '_' + str(args.repartition_iter) + '_mask_' + str(args.mask_ratio) + '_' + str(args.dataset)
    print("we are going to train from scratch.")
  
    if args.dataset == 'CIFAR10':
        raw_model = MLP3Layer(partition_num=args.world_size, model_size=args.model_size).to(device)
        partitioned_model = MLP3Layer(partition_num=1, model_size=args.model_size//args.world_size).to(device)
    elif args.dataset == 'RESISC45':
        raw_model = MLP3Layer(partition_num=args.world_size, model_size=args.model_size, label_num=45).to(device)
        partitioned_model = MLP3Layer(partition_num=1, model_size=args.model_size//args.world_size, label_num=45).to(device)
    elif args.dataset == 'AID':
        raw_model = MLP3Layer(partition_num=args.world_size, model_size=args.model_size, label_num=30).to(device)
        partitioned_model = MLP3Layer(partition_num=1, model_size=args.model_size//args.world_size, label_num=30).to(device)
...
#     optimizer = torch.optim.SGD(partitioned_model.parameters(), lr=args.lr)
    ...
    np.savetxt('./log/' + args.dataset + '/' + model_name + '_train_time.log', train_time_log, fmt='%1.4f', newline=' ')
    np.savetxt('./log/' + args.dataset + '/' + model_name + '_test_loss.log', test_loss_log, fmt='%1.4f', newline=' ')
    np.savetxt('./log/' + args.dataset + '/' + model_name + '_test_acc.log', test_acc_log, fmt='%1.4f', newline=' ')
    torch.save(raw_model, './trained_models/' + model_name + '.pth')


def main():
    parser = argparse.ArgumentParser(description='PyTorch 3-layer MLP on my datasets (subnet single PS)')
...
#                         help='model size for intermediate layers (default: 4096)')
    parser.add_argument('--model-size', type=int, default=1024, metavar='N',
                        help='model size for intermediate layers (default: 1024)')
...
#     parser.add_argument('--mask-ratio', type=float, default=0.0, metavar='N',
#                         help='mask ratio (default:0.0)')
    parser.add_argument('--mask-ratio', type=float, default=0.8, metavar='N',
                        help='mask ratio (default:0.9)')
#     parser.add_argument('--dataset', type=str, default='RESISC45', metavar='S',
#                         help='Dataset (default:RESISC45)')
#     parser.add_argument('--dataset', type=str, default='CIFAR10', metavar='S',
#                         help='Dataset (default:RESISC45)')
    parser.add_argument('--dataset', type=str, default='AID', metavar='S',
                        help='Dataset (default:RESISC45)')
...
if __name__ == '__main__':
    mp.set_start_method('spawn')  ### only for CLS_DATASET
    main()